...
st.title('FigmaChain')
st.subheader('Rendered Design from Figma')

# Define the chatbot template
template = """Assistant is a expert coding developer. Assistant only writes new code and does not write additional text.
...
def generate_response(prompt, html_content):
    completion = chatgpt_chain.predict(
        human_input=(
            "You are an excellent developer. I want you to improve the following code with the following {prompt}: {html_content}. Only output new html/css code. Do not write additional text."
...
if user_input:
    ...
    if is_valid_html_css_code(output):
        # Update the HTML content with the new output
